...
class LeadCaptureService:
    ...
    The service discovers and ranks leads, then returns them
    to the caller.
    """
    ...
    async def discover(
        ...
        strategy = (
            await self.strategy_engine.generate_strategy(
                keywords=keywords,
                location=location,
            )
        )
...
for result in completed:
    ...
    def _build_lead(
        ...
        return LeadRecord(
            ...
            city=data.get("city") or location,
    ...
    def _rank(
        ...
        if matched_developer_terms:
            score += 35
            reasons.append(
                "Business appears explicitly related "
        ...
        if data.get("address"):
            score += 10
            reasons.append(
                "Business location/address available."
            )

        if phones:
            score += 15
            reasons.append(
                "Business phone contact discovered."
            )

        if emails:
            score += 15
            reasons.append(
                "Business email contact discovered."
            )

        if data.get("linkedin_url"):
            score += 5
            reasons.append(
                "LinkedIn presence discovered."
            )

        if len(sources) >= 2:
            score += 5
            reasons.append(
                "Business discovered through "
                "multiple sources."
            )
