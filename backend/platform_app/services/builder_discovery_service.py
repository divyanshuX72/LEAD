...
def extract_geography(
    ...
    Handles addresses such as:
        ...
        Vevoor, Palghar, Maharashtra 401404, India

        Palghar West, Palghar, Maharashtra 401404, India
    """
    ...
    # City
    #
    # Use the closest meaningful component before the state.
    # --------------------------------------------------------
    ...
    if state_index is not None:
        ...
        for index in range(
            ...
            if postal_code and re.fullmatch(
                r"\d{6}",
                value,
            ):
                continue
...
# These are strong enough to identify a builder/developer.
STRONG_BUILDER_TERMS = (
    "builder",
    "builders",
    "developer",
    "developers",
...
# These provide supporting evidence but are NOT sufficient
# on their own to classify a company as a builder.
SUPPORTING_BUILDER_TERMS = (
...
# These indicate that the business may be an intermediary
# or another professional service rather than a builder.
NON_BUILDER_TERMS = (
    "real estate agent",
    "real estate agency",
    "property agent",
...
class BuilderDiscoveryService:
    ...
    async def discover(
        ...
        *,
        ...
        # --------------------------------------------------------
        # Search exhausted before target
        # --------------------------------------------------------
    ...
    async def _enrich_candidate(
        ...
        # ----------------------------------------------------
        # Builder activity evidence
        # ----------------------------------------------------

        positive_terms = [
            term
            for term in STRONG_BUILDER_TERMS
        ...
        supporting_terms = [
            term
            for term in SUPPORTING_BUILDER_TERMS
            if normalize_text(term)
            in normalized_text
        ]
            ...
            candidate.evidence.append(
                {
                    "source_type": "website",
                    "source_url": candidate.website,
                    "field": "supporting_activity",
                    "value": ", ".join(
                        supporting_terms
                    ),
                    "reason": (
                        "Website contains supporting "
        ...
        # ----------------------------------------------------
        # Recalculate geography from normalized address
        # ----------------------------------------------------
    ...
    def _qualify_candidate(
        ...
        # ----------------------------------------------------
        # Explicit builder/developer evidence
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Non-builder evidence
        # ----------------------------------------------------

        negative_matches = [
            term
            for term in NON_BUILDER_TERMS
            if normalize_text(term)
            in combined
        ]

        # ----------------------------------------------------
        # Strong builder evidence wins
        # ----------------------------------------------------

        if strong_matches:

            candidate.qualification_reasons.append(
                "Strong builder/developer evidence "
                "found: "
            ...
            if supporting_matches:
                ...
                candidate.qualification_reasons.append(
                    ...
                    "found: "
                    f"{', '.join(supporting_matches)}."
                )

            return True

        # ----------------------------------------------------
        # Supporting evidence alone is insufficient
        # ----------------------------------------------------
        ...
        if supporting_matches:
            ...
            if negative_matches:
                ...
                candidate.qualification_reasons.append(
                    "Candidate has supporting "
                    "construction/development evidence "
                    "but also has non-builder signals: "
                ...
                candidate.qualification_reasons.append(
                    "Only supporting construction/"
                    "development evidence was found; "
                    "no explicit builder/developer "
